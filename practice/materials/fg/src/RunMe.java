...
public class RunMe {
	...
	public static void main(String[] args) {
		...
		double ty = 1.0/(480);
//		double ty = 1.0/(480*1.2);

		PrintWriter out = new PrintWriter(System.out);
		
		for (int i = 0; i < (int) (480); i++) {
			
			double py = -2.0 + 4.0 * ty;
			int py_scr = (int) ( Math.abs((py - 2.0)) * (480.0 / 4.0) ); // translate to Image y-ordinate, length of [-2, 2]

			double tx = 1.0/(640);
//			double tx = 1.0/(640*1.2);

			for (int j = 0; j < (int) (640); j++) {
				
				double px = -2.0 + 4.0 * tx;
				int px_scr = (int) ( (px + 2.0) * (640.0 / 4.0) ); // translate to Image x-abscissa, length of [-2, 2]
...
//				} else if (r <= 10) { // outside ... (rapid move we have)
//					bi.setRGB(px_scr, py_scr, 0xFFFFFF);
//				} else if (r == 11) {
